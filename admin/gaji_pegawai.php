...
<body>
	...
	<table id="myTable">
		...
		<?php
			...
			$tes = $koneksi->query("SELECT pegawai.nama, gaji_pegawai.nama_sekolah, gaji_pegawai.bulan, COUNT(banyak_pertemuan) AS pertemuan, gaji, SUM(gaji) AS Total FROM gaji_pegawai JOIN pegawai ON pegawai.nip=gaji_pegawai.nip AND gaji_pegawai.nip='$nip' AND bulan='$bulan' AND tahun='$tahun' GROUP by nama_sekolah");
			$totalgaji = $koneksi->query("SELECT SUM(banyak_pertemuan*gaji) AS Total_Gaji FROM gaji_pegawai where nip='$nip' AND bulan='$bulan' AND tahun='$tahun'");
			if($tes) {
				if($tes->rowcount()>0) {
					while($data = $tes->fetch()) {
						echo'<tr>';
				...
				elseif($tes->rowcount()==0) {
					echo '<script language="javascript">alert("Gaji Belum Tersedia"); document.location="gaji.php";</script>';
				}
			}
			if($totalgaji) {
