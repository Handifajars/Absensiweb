    ...
    <table id="example" class="table table-striped table-bordered">
      <thead>
        <tr>
          ...
          <th>Nomor HP</th>
          <th><center>Gaji Pegawai</center></th>
        </tr>
      </thead>
      <tbody>
        <?php
        $query = $koneksi->query("SELECT nip,nama,jenis_kelamin,no_hp FROM pegawai ORDER BY nip ASC");
        if($query->rowcount()>0){
          $no = 1;
          while($data = $query->fetch()){
            ...
            echo'<td><center><a href="gaji_bulan.php?nip='.$data['nip'].'"onclick="return confirm(\'Yakin?\')">Detail</a></td>';
            echo'</tr>';
            $no++;
          }
        }
