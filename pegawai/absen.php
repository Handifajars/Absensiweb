...
<body>
	...
	<table id="myTable">
		...
		<?php
			...
			$query = $koneksi->query("SELECT jadwal.nama_sekolah, jadwal.hari, jadwal.jam FROM jadwal INNER JOIN pegawai ON jadwal.nip=pegawai.nip AND jadwal.nip='$username' AND jadwal.hari='$dayList[$day]'");
			...
			if($query->rowcount()>0) {
				...
				elseif($selisih!=0) {
					if($jam>=$selisih && $jam<=$waktu) {
            			...
            			echo'<td data-header="Status Absen">
            					<select name="status_absen">
									<option value="Masuk">Masuk</option>
						  			<option value="Libur">Libur</option>
						  		</select>
            				</td>';
            			echo'<td data-header="Absen Masuk"><input type="submit" name="Submit" value="Absen Masuk" ></td>';
        				echo'</tr>';
					}
					else{
						echo '<script language="javascript">alert("Belum Waktunya Absen"); document.location="index.php";</script>';
